    ...
    void OnTriggerStay(Collider other)
    {
		...
		if (other.gameObject.tag == "Player")
        {
            ...
            if (angle < fieldOfViewAngle * 0.5f)
            {
                ...
                Debug.DrawLine(transform.position + (transform.up), player.transform.position, Color.yellow);

                // ... and if a raycast towards the player hits something...
                if (Physics.Raycast(transform.position + (transform.up) , direction.normalized, out hit, 20.0f))
                {
