...
public class BlackMaskController : Pathfinding {
	...
	private void FixedUpdate() {
		...
		// 3 && 4 special punches 
			...
			GameObject player = GameObject.Find ("Eve");
	...
	private void Run() {
		if (Mathf.Abs (forwardInput) > inputSetting.inputDelay || Mathf.Abs (depthInput) > inputSetting.inputDelay) {
			cAnimator.SetFloat("VSpeed", Mathf.Abs (forwardInput) + Mathf.Abs (depthInput));
	...
	private void Jump() {
		if (jumpInput > 0 && Grounded ()) {
			...
			cAnimator.SetBool("Jumping", false);
			velocity.y = 0;
		}
		else {
			// decrease velocity.y
